...
def frommain(tag , sentence):
    ...
    if intent == 'vendor-name':
        ...
        vendor(sentence)
        print('b1')
        return vendor(sentence)
        print('c')
    if intent == 'contract-type':
        return contracttype(sentence)
    if intent == 'make-call':
...
def vendor(sentence):
    ...
    final_response = "vendor"
    try:
        name = nameSlot(sentence)
        # print "calling " + name[0]
        if(len(name)>1):
            final_response = "fetching vendor from " + name[1]
        else:
            final_response = "fetching vendor from " + name[0]
    except Exception:
        pass
    return final_response

def contracttype(sentence):
    # times = timeSlot(sentence)
    final_response = "type"
    try:
        name = nameSlot(sentence)
        # print "calling " + name[0]
        if(len(name)>1):
            final_response = "fetching type from " + name[1]
        else:
            final_response = "fetching type from " + name[0]
    except Exception:
        pass
    return final_response

#def contracttype(sentence):
#    final_response = "fetching contract type"
#    from_lookup = ["from"]
